...
public class GatewayAgencyCommandServiceImpl implements GatewayAgencyCommandService {
    ...
    @Inject
    public GatewayAgencyCommandServiceImpl(UserService userService, AgencyTravelService agencyTravelService,
            AirlineReservationCommandService airlineReservationCommandService,
            HotelBookingCommandService hotelBookingCommandService) {
    ...
    @Override
    public HotelFlightReservationDTO getHotelAndFlightReservationWithLockMode(long hotelBookingId,
            long flightReservationId) {
        ReservationDTO reservationDTO = this.airlineReservationCommandService
                .getAirlineReservationWithLockMode(flightReservationId);
    ...
    @Override
    public AgencyReservationSuccessDTO makeReservationFlight(final long idCustomer, final String dni,
            Map<Long, Integer> mapIdFlightInstance) {
        UserDTO userDTO = this.userService.readUserById(idCustomer);
        List<IdFlightWithSeatsDTO> idFlightWithSeatsDTOs = mapIdFlightInstance.entrySet().stream()
                .map(entry -> new IdFlightWithSeatsDTO(entry.getKey(), entry.getValue())).toList();
        AirlineReservationDTO airlineMTAReservationDTO = this.airlineReservationCommandService.makeReservation(userDTO,
                dni, idFlightWithSeatsDTOs);
        airlineMTAReservationDTO.getStatusFlightsDTO()
                .forEach(status -> this.agencyTravelService.makeTravel(
                        TravelMapper.INSTANCE.toMakeReservationAirline(airlineMTAReservationDTO, status, idCustomer)));
    ...
    @Override
    public AgencyReservationSuccessDTO makeReservationHotelAndFlight(final MakeBookingReservationDTO booking,
            long idCustomer, String dni, Map<Long, Integer> mapIdFlightInstance) {
        UserDTO userDTO = this.userService.readUserById(idCustomer);
        List<IdFlightWithSeatsDTO> idFlightWithSeatsDTOs = mapIdFlightInstance.entrySet().stream()
                .map(entry -> new IdFlightWithSeatsDTO(entry.getKey(), entry.getValue())).toList();
        BookingDTO bookingHotel = this.hotelBookingCommandService.makeBooking(booking, userDTO, dni);
        AirlineReservationDTO airlineMTAReservationDTO = this.airlineReservationCommandService.makeReservation(userDTO,
                dni, idFlightWithSeatsDTOs);
        airlineMTAReservationDTO.getStatusFlightsDTO()
                .forEach(status -> this.agencyTravelService.makeTravel(TravelMapper.INSTANCE
                        .toMakeReservationAirlineAndHotel(bookingHotel, airlineMTAReservationDTO, status, idCustomer)));
        return AgencyReservationSuccessDTO.builder()
                .dateReservation(airlineMTAReservationDTO.getDate())
    ...
    @Override
    public UpdateReservationDTO modifyReservationFlight(final long idReservation,
            final Map<Long, Integer> idFlightInstanceWithSeatsMap) {
        UpdateReservationDTO updateReservationDTO = this.airlineReservationCommandService
                .modifyReservation(idReservation, idFlightInstanceWithSeatsMap);
        ...
        listTravel.forEach(travel -> {
            ...
            final double price = updateReservationDTO.getUpdatePriceReservationDTO().stream()
                    .filter(f -> f.getIdFlightInstance() == idTravelFlight).findFirst().get().getPrice();
            if (numberSeats == 0)
    ...
    @Override
    public UpdateReservationDTO modifyHotelFlightReservation(final ModifyBookingReservationDTO booking,
            final long idReservation, final Map<Long, Integer> idFlightInstanceWithSeatsMap) {
        UpdateReservationDTO updateReservationDTO = this.airlineReservationCommandService
                .modifyReservation(idReservation, idFlightInstanceWithSeatsMap);
        BookingDTO bookingHotel = this.hotelBookingCommandService.modifyBooking(booking);
        List<TravelDTO> listTravel = this.agencyTravelService.findTravelByIdReservationFlightAndHotel(idReservation,
                booking.getId());
        if (listTravel.isEmpty())
            throw new TravelException("No se encontraron viajes asociados a la reserva de vuelo");
        listTravel.forEach(travel -> {
    ...
    public double cancelReservationFlight(final long idReservationFlight) {
        ...
        if (listTravel.isEmpty())
            throw new TravelException("No se encontraron viajes asociados a la reserva de vuelo");
        boolean isAllTravelFlightCancel = listTravel.stream()
                .map(travel -> this.agencyTravelService.cancelTravel(travel.getId())).allMatch(isCancel -> isCancel);
    ...
    public double cancelHotelFlightReservation(long hotelBookingId, long flightReservationId) {
        ...
        List<TravelDTO> listTravel = this.agencyTravelService
                .findTravelByIdReservationFlightAndHotel(flightReservationId, hotelBookingId);
        if (listTravel.isEmpty())
            throw new TravelException("No se encontraron viajes asociados a la reserva de vuelo");
        boolean isAllTravelFlightCancel = listTravel.stream()
                .map(travel -> this.agencyTravelService.cancelTravel(travel.getId())).allMatch(isCancel -> isCancel);
